...
class BasicBneck(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        x = self.se(x)
        x = self.project(x)
        result = residual + torchvision.ops.stochastic_depth(input=self.alpha * x,
                                                             p=self.drop_path,
                                                             mode="row",
...
class MoViNet(nn.Module):
    def __init__(self,
        ...
        for i, block in enumerate(cfg.blocks):
            num_basic_blocks += len(block)
        drop_path_rate = torch.linspace(0, max_drop_path_rate, num_basic_blocks)
        block_inx = 0
        for i, block in enumerate(cfg.blocks):
            for j, basicblock in enumerate(block):
                blocks_dic[f"b{i}_l{j}"] = BasicBneck(basicblock,
                                                      ...
                                                      norm_layer=norm_layer,
                                                      activation_layer=activation_layer,
                                                      drop_path=drop_path_rate[block_inx].item())
                block_inx += 1
